    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-surface w-full max-w-2xl rounded-2xl shadow-xl flex flex-col max-h-[90vh]">
        <div className="flex justify-between items-center p-6 border-b border-outline-variant">
          <h2 className="font-headline-sm text-headline-sm text-on-surface font-bold">Add New Lead</h2>
          <button onClick={() => navigate(-1)} className="text-on-surface-variant hover:text-on-surface p-2 rounded-full hover:bg-surface-container-high transition-colors">
            <span className="material-symbols-outlined">close</span>
          </button>
        </div>
        <div className="p-6 overflow-y-auto">
          <form id="add-lead-form" onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="p-6 border-t border-outline-variant flex justify-end gap-3">
          ...
          <button type="submit" form="add-lead-form" className="px-6 py-2 rounded-lg font-label-bold text-on-primary bg-primary hover:bg-primary/90 transition-colors">
            Save Lead
          </button>
        </div>
      </div>
